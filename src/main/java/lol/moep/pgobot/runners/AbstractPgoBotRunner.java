...
public abstract class AbstractPgoBotRunner implements PgoBotRunner {
        ...
        for (GeoCoordinate c : interpolatedCoordinates) {
            ...
            if (coordinatesVisited % (this.moveActionDistance / 10) == 0) {
                try {
                    ...
                    this.sc.logError(e.getMessage(), e);
                }
            }
        }
    ...
    protected void findAndCatchPokemon() {
        ...
        try {
            ...
            this.sc.logError("Serverfehler: Pokémon in der Nähe konnten nicht ermittelt werden.", e);
            return;
        }
        ...
        for (CatchablePokemon p : pokemons) {
            try {
                ...
                this.sc.logError("Serverfehler beim Fangen von " + p.getPokemonId().name(), e);
            }
...
//                    }
    ...
    private boolean isBreeding() {
		try {
			final List<EggIncubator> incubators = this.go.getInventories().getIncubators();
